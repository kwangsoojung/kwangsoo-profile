...
export function HeroVisual({ className = '', visual }: HeroVisualProps) {
  return (
    <div
      className={`hero-visual pointer-events-none relative flex min-h-[28rem] items-center justify-center overflow-visible bg-transparent sm:min-h-[34rem] ${className}`}
    >
      <AssetImage
        alt={visual.background.alt}
        className="hero-visual-bg absolute left-1/2 top-[4%] z-10 h-[56%] w-[112%] -translate-x-1/2 object-contain opacity-45 mix-blend-multiply"
        fallback={null}
        src={visual.background.src}
      />
      <AssetImage
        alt={visual.portrait.alt}
        className="hero-portrait relative z-20 block h-auto w-[min(92vw,480px)] max-w-none object-contain sm:w-[clamp(420px,42vw,720px)] lg:w-[clamp(560px,52vw,860px)]"
        fallback={
